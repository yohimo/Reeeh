...
class Database 
{
    ...
    static public function getInstance() 
    {
        if(!self::$PDOInstance) { 
	        try {
                self::$PDOInstance = new PDO("mysql:host=localhost;dbname=reeeh","root","",array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8",PDO::ATTR_ERRMODE=> PDO::ERRMODE_EXCEPTION ));
    ...
    {
        return self::$PDOInstance->fetchAll($SQL);
    }
    */
...
returned  
    ...
    public function Query($statement) {
    	return self::$PDOInstance->query($statement)->fetchAll();
    }
    


    public function FArray($statement) 
    {
        return self::$PDOInstance->fetch($statement);
    }
